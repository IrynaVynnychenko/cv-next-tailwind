...
export default function ServiceCards() {
  ...
  const lastRowStart = SERVICE_ORDER.length - (SERVICE_ORDER.length % 2 === 0 ? 2 : 1)
    ...
    <Panel id="services" title={homeServices.title[language]}>
      ...
      <div className="grid md:grid-cols-2">
        {SERVICE_ORDER.map((id, index) => {
          const s = services[id]
          const borders = [
            index < SERVICE_ORDER.length - 1 ? 'border-b border-edge' : '',
            index % 2 === 0 ? 'md:border-r md:border-edge' : '',
          ...
          return (
            <Link
              ...
              className={`group flex flex-col gap-2 p-4 transition-colors hover:bg-accent-muted ${borders}`}
            >
